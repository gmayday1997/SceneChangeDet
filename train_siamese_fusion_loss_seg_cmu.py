...
def single_layer_similar_heatmap_visual(output_t0,output_t1,save_spatial_att_dir,epoch,filename,layer_flag):
    ...
    save_spatial_att_dir_layer = os.path.join(save_spatial_att_dir_,layer_flag)
    check_dir(save_spatial_att_dir_layer)
    #save_weight_fig_dir = os.path.join(save_spatial_att_dir_layer, real_name[:real_name.find('.')] + '_spatial.jpg')
    save_weight_fig_dir = os.path.join(save_spatial_att_dir_layer,filename + '.jpg')
...
def validate(net, val_dataloader,epoch,save_valid_imags_dir,save_spatial_att_dir,save_roc_dir):
    ...
    net.eval()
    gts, preds = [], []
    metric_for_class = util.init_metric_for_class(number_class='2')
    for batch_idx, batch in enumerate(val_dataloader):
        ...
        for i in range(len(prob_seg)):
            ...
            metric_for_class[i]['total_fp'] += FP
            metric_for_class[i]['total_fn'] += FN
            metric_for_class[i]['total_posnum'] += posNum
            metric_for_class[i]['total_negnum'] += negNum
        ...
        #check_dir(root_dir),check_dir(sub_dir)
        #save_fig_dir = os.path.join(sub_dir, real_name[:real_name.find('.')] + '_seg.jpg')
        save_fig_dir = os.path.join(save_valid_dir, filename + '.jpg')
    ...
    for i in range(2):
        ...
        metric_dict.setdefault('iu',class_iou[i])
        metric_for_class[i].setdefault('metric', metric_dict)

    #return score['Mean IoU :']
    save_metric_epoch_dir = os.path.join(save_roc_dir,str(epoch))
    check_dir(save_metric_epoch_dir)
    #save_metric_dir = os.path.join(save_metric_epoch_dir,'metric')
    #check_dir(save_metric_dir)
    mc.save_metric2disk(metric_for_class,save_metric_epoch_dir)
    f_score_total = 0.0
    for i in range(2):
        pr, recall,f_score = metric_for_class[i]['metric']['precision'], metric_for_class[i]['metric']['recall'],metric_for_class[i]['metric']['MaxF']
...
def main():
  ...
  if base_seg_model == 'deeplab':
      ...
      model = models.SiameseNet(class_number=2,norm_flag='l2')
      if resume:
  ...
  save_spatial_att_dir = os.path.join(cfg.SAVE_PRED_PATH, 'various_spatial_att_scale/')
  check_dir(save_spatial_att_dir)
  save_valid_dir = os.path.join(cfg.SAVE_PRED_PATH,'various_valid_imgs_scale')
  check_dir(save_valid_dir)
  save_roc_dir = os.path.join(cfg.SAVE_PRED_PATH,'ROC_scale')

...
class SiameseNet(nn.Module):
    ...
    def __init__(self,class_number=2,norm_flag='l2'):
        super(SiameseNet, self).__init__()

        self.CNN = deeplab_V2_partial()
        self.fc8 = nn.Conv2d(in_channels=2048,out_channels=class_number,kernel_size=1)
        #self.norm_conv = nn.BatchNorm2d(512)
        #self.norm_fc = nn.BatchNorm2d(1024)
        ...
        if norm_flag == 'l1':
            self.norm = fun.l2normalization(scale=1)
        self.scale = fun.scale_feature(scale=1)

    def forward(self,t0,t1):

        feat_conv5_t0,feat_fc_t0 = self.CNN(t0)
        feat_conv5_t1,feat_fc_t1 = self.CNN(t1)
        out_conv5_t0,out_fc_t0 = self.norm(feat_conv5_t0),self.norm(feat_fc_t0)
        out_conv5_t1,out_fc_t1 = self.norm(feat_conv5_t1),self.norm(feat_fc_t1)
        #out_conv5_t0,out_fc_t0 = self.softmax(feat_conv5_t0),self.softmax(feat_fc_t0)
        #out_conv5_t1,out_fc_t1 = self.softmax(feat_conv5_t1),self.softmax(feat_fc_t1)
        fc = self.scale(torch.cat((out_fc_t0,out_fc_t1),dim=1))
        seg_branch = self.fc8(fc)
        '''''''''
        out_conv5_t0,out_fc_t0 = self.scale(self.norm(feat_conv5_t0)),self.scale(self.norm(feat_fc_t0))
        out_conv5_t1,out_fc_t1 = self.scale(self.norm(feat_conv5_t1)),self.scale(self.norm(feat_fc_t1))
        fc = torch.cat((out_fc_t0,out_fc_t1),dim=1)
        seg_branch = self.fc8(fc)
        '''''''''
        seg_score = F.upsample(seg_branch,t0.size()[2:],mode='bilinear')
        #out_conv5_t0,out_fc_t0 = self.softmax(feat_conv5_t0),self.softmax(feat_fc_t0)
        #out_conv5_t1,out_fc_t1 = self.softmax(feat_conv5_t1),self.softmax(feat_fc_t1)
        return seg_score,[out_conv5_t0,out_conv5_t1],[out_fc_t0,out_fc_t1]
